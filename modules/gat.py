...
from modules.layers import GAT

from ._utils import BBAccuracy


class MIPPLModelGAT(pl.LightningModule):
    def __init__(
        self,
        arch: str,
        nheads: int,
        ...
        super(MIPPLModelGAT, self).__init__()
        ...
        self.model = GAT(
            nfeat=19, nhid=emb_size, nclass=1, dropout=0.0, nheads=nheads, alpha=0.2
        ).to(self.device)
    ...
    def _pad_adj(self, edge_index, edge_attr, num_vars, num_cons):
    ...
    def training_step(self, batch, _):
        ...
        adj = self._pad_adj(
            batch.edge_index,
            batch.edge_attr,
            batch.variable_features.size(0),
            batch.constraint_features.size(0),
        )
